...
public class yahoo {
...

	{
		try {
			for (int i = 0; i < no_comp; i++) {
				URL comp = new URL(
						"http://in.finance.yahoo.com/d/quotes.csv?s=" + s[i]
								+ "&f=sl1pj3d1t1");
				...
				while ((inputLine = rd2.readLine()) != null) {
					...
					s1 = inputLine.split(",");
					System.out.println("Ticker No-" + (i + 1) + " Ticker-"
							+ s1[0] + " Current price-" + s1[1]
							+ " Previous Close-" + s1[2] + " Market Cap-"
							+ s1[3] + " Date-" + s1[4] + " Time-" + s1[5]);
					if ((System.currentTimeMillis() - r) >= (R * 1000)) {
						RandomAccessFile file = new RandomAccessFile(
								"c:/java/yahoo.txt", "rwd");
						// file.setLength(0);
						BufferedWriter bw = new BufferedWriter(new FileWriter(
								"c:/java/yahoo.txt", true));

						bw.write("    " + "current price-" + s1[1] + ","
								+ "previous close-" + s1[2] + ","
								+ "market cap-" + s1[3] + "       ");
						bw.close();
